...
async function processLearningEntry(chatId, content, category = null, generateFollowUp = true) {
    ...
    const validation_result = validation.validateLearningContent(content);
    if (!validation_result.valid) {
        await bot.sendMessage(chatId, `❌ ${validation_result.error}`);
        return;
    }

    const sanitizedContent = validation_result.sanitized;

    // Validate category if provided
    if (category) {
        const categoryValidation = validation.validateCategory(category);
        if (!categoryValidation.valid) {
            category = null; // Reset to null if invalid
        } else {
            category = categoryValidation.category;
        }
    }

    await db.addLearning(chatId, sanitizedContent, category);
...
bot.onText(/\/search(?: (.+))?/, async (msg, match) => {
    ...
    const query = match[1];
    if (!query) {
        await bot.sendMessage(chatId, "🔍 *Search Your Learnings*\n\nWhat topic would you like to search for?", { parse_mode: 'Markdown', reply_markup: { force_reply: true, input_field_placeholder: "Search for..." } });
        userStates[chatId] = { command: 'search' };
        return;
    }
    // Validate search query
    const searchValidation = validation.validateSearchQuery(query);
    if (!searchValidation.valid) {
        await bot.sendMessage(chatId, `❌ ${searchValidation.error}`);
        return;
    }
    const results = await db.searchLearnings(chatId, searchValidation.sanitized);
    if (results.length === 0) {
        await bot.sendMessage(chatId, `🔍 No learnings found for "${validation.escapeHtml(searchValidation.sanitized)}"`);
        return;
    }
    const formattedResults = results.map((l, i) => `${i + 1}. ${l.content}\n   📅 ${format(new Date(l.createdAt), 'MMM dd, yyyy')}`).join('\n\n');
    await bot.sendMessage(chatId, `🔍 *Search Results for "${validation.escapeHtml(searchValidation.sanitized)}"*\n\n${formattedResults}`, { parse_mode: 'Markdown' });
});
...
bot.on('message', async (msg) => {
    ...
    try {
        switch (state.command) {
            ...
            case 'search':
                const searchVal = validation.validateSearchQuery(msg.text);
                if (!searchVal.valid) {
                    await bot.sendMessage(chatId, `❌ ${searchVal.error}`);
                    delete userStates[chatId];
                    break;
                }
                const results = await db.searchLearnings(chatId, searchVal.sanitized);
                if (results.length === 0) {
                    await bot.sendMessage(chatId, `🔍 No results found for "${validation.escapeHtml(searchVal.sanitized)}"`);
                } else {
                    const formattedResults = results.slice(0, 10).map((l, i) => `${i + 1}. ${l.content}\n   📅 ${format(new Date(l.createdAt), 'MMM dd')}`).join('\n\n');
                    await bot.sendMessage(chatId, `🔍 *Found ${results.length} result(s) for "${validation.escapeHtml(searchVal.sanitized)}"*\n\n${formattedResults}${results.length > 10 ? '\n\n...and more' : ''}`, { parse_mode: 'Markdown' });
                }
                delete userStates[chatId];
                break;
            case 'custom_goal':
                const goalValidation = validation.validateGoal(msg.text);
                if (goalValidation.valid) {
                    await db.setUserGoal(chatId, goalValidation.value);
                    await bot.sendMessage(chatId, `🎯 Perfect! Your daily goal is set to ${goalValidation.value} learnings per day.`, { ...getMainMenuKeyboard() });
                    delete userStates[chatId];
                } else {
...
bot.on('polling_error', (error) => {
    ...
    console.error(`[Polling Error] ${error.code || 'UNKNOWN'}: ${error.message || 'Unknown error'}`);
});
