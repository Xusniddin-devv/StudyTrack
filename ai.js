...
// Generate next quiz question for inline quiz mode
async function getNextQuizQuestion(learnings, conversationHistory = []) {
    if (!learnings || learnings.length === 0) {
        return null;
    }
    ...
    const systemPrompt = `You are a thoughtful quiz master. Based on the user's learnings and our conversation, ask ONE insightful question to test their understanding.

Guidelines:
- Ask open-ended questions that test comprehension, not just recall
- Build on previous questions if there are any
- Keep questions clear and concise
- Don't repeat topics already covered
- Be encouraging and educational

Just provide the question - no extra formatting or labels.`;

    let userPrompt = `Based on these learnings:\n\n${learningText}\n\n`;

    if (conversationHistory.length > 0) {
        userPrompt += `Previous conversation:\n${conversationHistory.map(msg => `${msg.role}: ${msg.content}`).join('\n')}\n\n`;
    }

    userPrompt += 'Ask the next quiz question.';
